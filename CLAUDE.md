...
- **KYC is no longer categorically shelved as of 2026-08-04.** A one-shot, OTP-assisted registration+KYC-submission flow (`maestro/flows/kyc/login_and_submit_kyc.yaml`) now exists and has been run successfully against a dedicated test account. It is explicitly a single-use flow, not a repeatable regression flow (KYC can't be resubmitted for the same account) — do not apply the 5-consecutive-runs bar to it. "Skip KYC" and "KYC saved as draft after skip" remain blocked: they need a fresh, not-yet-KYC'd test account, which isn't currently available.
- **OTP entry requires a human present to read the SMS and type the code in real time** — there is no test-mode bypass or backend access to read OTPs programmatically. This is a standing capability gap, not necessarily a staging outage. Any flow needing fresh OTP verification (new registration, wallet activation, etc.) is blocked for this reason until a bypass exists.
- **Logout carries a real, standing risk**: the primary physical test device has one working session, and logging out would require a human-assisted OTP re-login (see above) to recover. Treat any Logout automation the same as the Band 2 risk items below — verify the control is reachable, but do not tap it without the user's explicit, in-the-moment go-ahead, regardless of how a given task's instructions categorize it.
